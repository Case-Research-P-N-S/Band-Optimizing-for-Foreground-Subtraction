...
def blackbodyConvertofNu(nu, const):
    ...
    h, c, k, TVac, TDust = const[:]
    return 2*(h**2)*(nu**4)*np.exp((h*nu)/(k*TVac))/(k*(TVac**2)*(c**2)*((np.exp((h*nu)/(k*TVac)) - 1)**2))


def blackbody(nu, const):                                                           # Why is blackbody function never called?
...
def dustRatio(nu1, nu2, nu0, const):
    ...
    return (dustFreqPowLaw(nu1, nu0)*blackbodyConvertofNu(nu2, const))/(dustFreqPowLaw(nu2, nu0)*blackbodyConvertofNu(nu1, const))
